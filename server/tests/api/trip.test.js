...
describe('Trip endpoints', () => {
  ...
  describe('POST "/api/v1/trips" endpoint', () => {
    it('"/api/v1/trips" should throw 400 BAD request error', () => (
      request.post('/api/v1/trips')
        ...
        .expect(400)
        .then(async (response) => {
          expect(response.error.text).toMatch(' must have required property \'long\'');
        })
    ));
  ...
  describe('POST "/api/v1/trips/:id/end" endpoint', () => {
